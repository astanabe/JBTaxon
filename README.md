...
以下にソースとなるチェックリストを全て掲載します。なお、複数のソース間で衝突する場合は、(1) より狭い分類群のソースを優先、(2) 次いでより新しいソースを優先します。ただし、学名の有効名とシノニムの判定には、Catalogue of Lifeを基本としつつ「より新しいソースか」だけを用い、より狭い分類群のソースかどうかは考慮しません。Catalogue of Lifeとその他ソースで学名の有効名が決められない場合、NCBI Taxonomyを使用します。和名に使用できる文字は、漢字、ひらがな、全角カナ、踊り字(々)、長音符(ー)、半角英数字、ギリシャ文字、記号(・(中黒)、-(ハイフン)、.(ピリオド))です。和名の有効名の判定には、種以下のレベルでは「カナ名」を必須(ただしカナ文字以外が付いてもよい)、「階層レベルを示す単語」は付いていても構わないものとし、種より上の高次分類群名では「カナ名」(ただしカナ文字以外が付いてもよい)と「階層レベルを示す単語」の組み合わせを必須とします。「階層レベルを示す単語」は「rank・subrankについて」の項で示してあります。その上で、複数のソース間で衝突する場合は、(1) より狭い分類群のソースを優先、(2) 次いでより新しいソースを優先します。
