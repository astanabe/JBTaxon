...
以下にソースとなるチェックリストを全て掲載します。なお、複数のソース間で衝突する場合は、(1) より狭い分類群のソースを優先、(2) 次いでより新しいソースを優先します。ただし、学名の有効名とシノニムの判定には、Catalogue of Lifeを基本としつつ「より新しいソースか」だけを用い、より狭い分類群のソースかどうかは考慮しません。Catalogue of Lifeとその他ソースで学名の有効名が決められない場合、NCBI Taxonomyを使用します。和名の有効名の判定には、種以下のレベルでは「カナ名」を必須とし、種より上の高次分類群名では「カナ名」と「階層レベルを示す単語」の組み合わせを必須とします。「階層レベルを示す単語」は「rank・subrankについて」の項で示してあります。その上で、複数のソース間で衝突する場合は、(1) より狭い分類群のソースを優先、(2) 次いでより新しいソースを優先します。
