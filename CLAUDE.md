...
- **和名から除くもの（全ソース共通、`norm_japname`）**: 空白、`？`、前置きの `別名：` `別称：`、`：` 以降の説明（`キツネゴケ：コケ植物と同名異種`）、`”` 以降（`タテゴケ属”はPsoraへの後続同名`）、日本語が続く `．` 以降（`エゾカラタチゴケ．和名保留`）、`【広義】`。区切り文字には `；` も含める。情報源の誤記で正しい表記が判明しているものは `%JAPNAME_FIX` で直す。
- **和名でないもの（`is_placeholder`）**: `〜の一種` `〜の一種の幼虫` `〜属の一種1`、`和名保留` `所属科不明`、`〜新称`（`名部みち代新称`）、4桁の年で終わるもの（`澤田1944`）。
...
- **作り直しは差分だけ行う。** ソースごとに `<分類群>/.jbtaxon/<ソースID>.tsv.fp` に指紋を置き、一致していればパースを飛ばす。指紋に入れるのは (1) `VERSION` と `BUILDDATE`、(2) 本スクリプトの**共通部分**のダイジェスト、(3) **そのソースのパーサ部分**のダイジェスト、(4) `@SOURCES` のそのエントリ、(5) 入力ファイルの一覧・サイズ・更新時刻。統合も同様に `<分類群>/.jbtaxon/merge.fp` で判定し、中間 TSV と参照データ（`NameUsage.tsv` / `names.dmp`）が変わっていなければ飛ばす。
  - 共通部分とパーサ部分は `#---` の罫線でセクションに切り、**`sub parse_<名前>` をちょうど1つ定義しているセクションだけ**をそのパーサのものとみなす（`col_scan` や `jaflist_scinames` のようなパーサ固有の補助サブは同じセクションに置いてあるので一緒に扱われる）。判定できないものは共通部分に入るので、**取りこぼして作り直しを忘れることはない**（余分に作り直す方向に倒れる）。
  - したがって `norm_sciname` のような共通の関数を直すと全ソースを作り直し、1つのパーサだけを直すとそのソースだけを作り直す。実測で全生成 3分05秒に対し、無変更の再実行は 10秒、1ソースだけの作り直しも 10秒。
