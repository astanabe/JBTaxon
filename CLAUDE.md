...
- **括弧に入れた注記・引用は名前ではない**（`strip_japnotes`）。編集上の覚え書き（`（誤記）` `（統合）` `（二倍体）` `（註１）` `（下位同物異名予定）`）と、数字・ラテン文字・コロンを含む文献の引用（`(Hatus., 改訂鹿児島県植物目録: 230, 1986)`）を括弧ごと落とす。**引用は別名の並びの途中にも挟まる**ので、`；`や`，`で分ける前に落とすこと（`split_japnames` の先頭で行う）。
